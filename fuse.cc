...
#if 1
    bool flag = to_set & FUSE_SET_ATTR_SIZE;
    if (flag) chfs->setattr(ino, attr->st_size);
    getattr(ino, st);
    fuse_reply_attr(req, &st, 0);
#else
...
fuseserver_symlink(fuse_req_t req, const char * link, fuse_ino_t parent, const char * name) {
    ...
    chfs_client::status stat = chfs->symlink(parent, name, ino_out, link);
    ...
    if (getattr(ino_out, e.attr) != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
    fuse_reply_entry(req, &e);
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    std::string buf;
    chfs_client::status stat = chfs->readlink(ino, buf);
    if (stat != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
